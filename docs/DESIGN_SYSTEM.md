...
- **Photo-first storytelling**: Keep imagery primary and surround it with concise supporting copy. Reveal contextual metadata (map pins, EXIF, tags) progressively rather than all at once.
- **Progressive exploration**: Offer approachable previews—hero grids, inline maps, quick stats—that invite deeper dives without overwhelming first-time visitors. Pair each surface with a clear CTA.
- **Trust through clarity**: Reinforce authenticity with visible sources (camera, location, timeline) and calm typography. Use consistent iconography and rhythm to avoid visual noise.
- **Low-friction contribution**: In admin flows favor keyboard shortcuts, batch actions, and forgiving forms so uploading new trips or photos stays quick even at scale.

---

## 📱 Responsive Layout Patterns

| Breakpoint | Key Layout Rules | Notable Components |
|------------|------------------|--------------------|
| **Mobile ≤640px** | Single-column stack with generous spacing; sticky bottom nav; defer heavy map layers behind drawers. | TripCard, MobileFilters, Lightbox, BottomSheet |
| **Tablet 641-1024px** | Two-column grids for galleries; reveal inline filters; show map + list side-by-side for places. | SplitView, InlineMap, FilterBar |
| **Desktop ≥1025px** | Max width 1200-1440px; enable hover reveals, persistent sidebar navigation, multi-column editorial layouts. | HeroGrid, StorySidebar, StatsPanels |

- Hero sections use responsive typography: `text-3xl` mobile → `text-5xl` desktop (custom class).
- Map experiences swap to cluster view after 12+ markers and lazy-load heavy layers until interaction.
- Gallery masonry maintains aspect-awareness by loading square placeholders, then transitioning to true ratios to reduce layout shift.

---

## 🤝 Interaction Patterns

- Hover & focus parity: Any hover reveal (EXIF badges, overlay actions) must provide an equivalent focus-visible or tap interaction. Consider `group-focus-within` wrappers for keyboard parity.
- Filter persistence: Store active filters in query params so explorers can share URLs; mirror selections in chips above the content surface.
- Lightbox behavior: Arrow keys cycle media, `Esc` closes, `L` toggles like/save, and captions pin without obscuring the photo bottom edge.
- Map interactions: Pin clicks open a drawer rather than modals; drawers include quick actions (view trip, see nearby, add to collection).
- Feedback states: Provide skeletons for every data-heavy block, empty states with illustrative icons, and inline toasts for saves/deletes to avoid disruptive modals.
...
- Maintain WCAG AA contrast (4.5:1) for text; reserve accent orange for backgrounds or icons with white text ≥16px bold.
- Respect `prefers-reduced-motion` by disabling `slideUp` animations and swapping shimmer skeletons for static placeholders.
- Provide textual summaries for map-heavy sections so screen readers receive the same insights (e.g., total places, average rating, top tags).
- Validate forms with inline messaging and `aria-live="polite"` regions to announce feedback without shifting layout abruptly.
...
- [ ] Configure responsive typography utilities (mobile-first scaling)
...
- [ ] Implement lightbox keyboard/touch interactions
...
- [ ] Audit accessibility (contrast, reduced motion, aria-live)
- [ ] QA responsive layouts across mobile, tablet, desktop
